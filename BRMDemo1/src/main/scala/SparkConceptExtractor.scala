...
Serializable{
  ...
  .Map[String,Any]] ={
    ...
    val input = inputIndexMap.map(x=>(x._1.getOrElse("_source", "").asInstanceOf[Map[String, Any]].getOrElse("text",
      "").asInstanceOf[String],defineSubentities(x._1.getOrElse("_source", "").asInstanceOf[Map[String, Any]]
      .getOrElse("text", "").asInstanceOf[String]), x._2)) // origMap, submessages, index
    ...
    val result = inputIndexMap.map(x => (x._2, x._1)).cogroup(tempResult).map(x=> (x._2._1.iterator.next()
      ...
      val origMutableMap = collection.mutable.Map(x._1.toSeq: _*)
      ...
      val new_x = x._1 + ("_source" -> new_source)
      new_x
    })
