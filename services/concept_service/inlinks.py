...
class Inlinks(object):
    ...
    def __init__(self, text, inlinks_threeshold, concepts_inlinks,stopwords,entities,composition):
        '''
        Constructor
        '''
        self.text=text
        ...
        while index>i:
            ...
            if val!=None and s not in self.stopwords and int(val)>=self.inlink_threeshold and self.no_all_sw(s) and len(s)>1:
                ...
                if s in self.entities:
                    is_entity=True
                #results[s]={'inlinks':int(val),'start':left_position+1,'end':right_position,'entity':is_entity,'composition':composition}
